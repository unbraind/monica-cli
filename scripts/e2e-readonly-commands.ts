...
export function buildWriteGuardCommands(contactId: number | null): string[] {
  const checks = [
    ...
    "monica vaults create --name 'read-only-guard'",
...
export function buildBaseCommandChecks(input: BuildBaseCommandChecksInput): CommandCheck[] {
  ...
  return [
    ...
    { command: `monica ${baseFlags} api-research source-status --edition next`, outputValidation: 'json' },
    { command: `monica ${baseFlags} api-research openapi --edition stable`, outputValidation: 'json' },
    { command: `monica ${baseFlags} api-research openapi --edition next`, outputValidation: 'json' },
    { command: `monica --yaml api-research openapi --edition next --oas-version 3.1.2`, outputValidation: 'yaml' },
    { command: `monica ${baseFlags} api-research diff`, outputValidation: 'json' },
    { command: `monica ${baseFlags} api-research validate-contract --edition stable --fail-on-warnings`, outputValidation: 'json' },
    { command: `monica ${baseFlags} api-research validate-contract --edition next --fail-on-warnings`, outputValidation: 'json' },
    { command: `monica ${baseFlags} api-research coverage --source next --fail-on-unmapped`, outputValidation: 'json' },
    ...
    { command: `monica ${baseFlags} schemas get next-vault`, outputValidation: 'json' },
